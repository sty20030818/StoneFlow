...
pub(super) async fn sync(
    ...
    let source_items = Links::find()
        ...
        .all(source_db)
        .await
        .map_err(|error| format!("{}: {}", direction.read_source_error("Links"), error))?;
    ...
    let existing_versions: HashMap<String, i64> = if source_items.is_empty() {
        ...
        Links::find()
            ...
            .map_err(|error| {
                format!(
                    "{}: {}",
                    direction.read_target_existing_error("Links"),
                    error
                )
            })?
            .into_iter()
    ...
    for item in source_items {
        ...
        links::Entity::insert(active_model)
            ...
            .exec(target_db)
            .await
            .map_err(|error| format!("{}: {}", direction.write_target_error("Link"), error))?;
    }
