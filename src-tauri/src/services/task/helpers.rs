...
pub fn priority_to_value(priority: &Priority) -> String {
    match priority {
        Priority::P0 => "P0",
        Priority::P1 => "P1",
        Priority::P2 => "P2",
...
pub fn done_reason_to_value(done_reason: &Option<DoneReason>) -> Option<String> {
...
pub fn normalize_tags_for_log(tags: &[String]) -> Vec<String> {
    let mut normalized = Vec::new();
    for tag in tags {
        let value = tag.trim();
...
pub fn format_link_for_log(kind: &str, title: &str, url: &str) -> String {
    format!("{}:{}<{}>", kind, title.trim(), url.trim())
}

pub fn normalize_links_for_log(links: &[LinkInputDto]) -> Vec<String> {
    links
        .iter()
...
pub fn to_optional_join(values: Vec<String>, separator: &str) -> Option<String> {
    if values.is_empty() {
        None
    } else {
...
pub fn dedup_project_ids(mut project_ids: Vec<String>) -> Vec<String> {
    project_ids.sort();
    project_ids.dedup();
    project_ids
}
