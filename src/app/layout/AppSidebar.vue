<template>
  ...
  >
    ...
    <div class="px-3 pt-3 shrink-0">
      <div class="mb-2" :class="brandShellClass">
        <BrandLogo />
      </div>
      <UTabs
    ...
    <div class="px-3 pt-2 shrink-0">
      <section>
        ...
        <nav class="flex flex-col gap-0.5">
          ...
          >
            <UIcon :name="allTasksNav.icon" :class="allTasksNav.iconClass" />
          ...
          >
            <UIcon :name="projectIcon" class="size-3.5" :class="defaultProjectIconClass" />
          ...
          >
            <UIcon :name="trashNav.icon" :class="trashNav.iconClass" />
    ...
    <div class="shrink-0 border-t border-default/80 bg-default/80">
      ...
      <div class="px-3">
        ...
        >
          ...
          <Transition name="library-nav-shell">
            <nav v-if="!isLibraryCollapsed" class="flex flex-col gap-0.5 mt-1.5">
              <RouterLink
              ...
              >
                <UIcon :name="item.icon" :class="item.iconColor" />
      ...
      <!-- Auto Sync Card -->
      <div class="p-3" :class="isLibraryCollapsed ? 'pt-0' : 'pt-3'">
        <div v-motion="userCardMotion" class="pt-1.5">
          <AutoSyncCard />
        </div>
      </div>
    </div>
...
import { detectDesktopShellPlatform } from "@/app/layout/header/desktop-shell";
import {
  DraggableProjectTree,
...
const brandShellClass = computed(() => (shellPlatform === "mac" ? "pl-[4.5rem]" : ""));
...
const defaultProject = computed(
  () => currentProjects.value.find((p) => isDefaultProjectId(p.id)) ?? null,
);
...
const cachedProjectsTree = computed<ProjectTreeItem[]>(
  () => projectsTreeCacheBySpace.value[spaceValue.value] ?? [],
);
const displayProjectsTree = computed<ProjectTreeItem[]>(() => {
  if (projectsTree.value.length > 0) return projectsTree.value;
  if (isProjectsLoading.value && cachedProjectsTree.value.length > 0)
    return cachedProjectsTree.value;
...
watchDebounced(
  ...
  ([spaceId, projectId]) => {
    ...
    if (!projectTreeStore.hasMissingAncestorsInExpanded(spaceId, projectId, currentProjects.value))
      return;
