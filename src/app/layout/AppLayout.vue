<template>
	<AppHeaderProvider>
		<div class="h-full flex bg-default text-default relative overflow-hidden z-layer-base">
			<AppSidebar v-model:space="space" />
			...
			<!-- 右侧 Inspector Drawer 作为应用布局级挂载点统一托管。 -->
			<TaskInspectorDrawer />
...
<script setup lang="ts">
	...
	import { AppHeaderProvider } from '@/app/layout/header'
	import { useAppShellMotionPreset } from '@/shared/composables/base/motion'
	import GlobalBackToTopButton from '@/shared/ui/GlobalBackToTopButton.vue'
	import { SPACE_IDS, type SpaceId } from '@/shared/config/space'
	import { useStartupReady } from '@/app/startup/initialize'
	import {
		classifyRouteScope,
		extractWorkspacePayload,
		isWorkspaceRoute,
		toWorkspaceRouteTarget,
	} from '@/app/startup/route-memory-policy'
